...
class VisuoTactileSensor(SensorBase):
    ...
    def _find_body_handles(self):
        ...
        if hasattr(self.cfg, "indenter_rigid_body") and self.cfg.indenter_rigid_body is not None:
            ...
            indenter_pattern = f"{env_path}/{self.cfg.indenter_rigid_body}"
    ...
    def _initialize_sdf(self):
        ...
        prim_paths_expr = f"/World/envs/env_.*/{self.cfg.indenter_sdf_mesh}"
        num_query_points = self.cfg.num_tactile_rows * self.cfg.num_tactile_cols
        prepare_sdf_schemas = False  # indenter should already have an SDF collision mesh

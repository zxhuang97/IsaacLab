...
usd_path = os.path.abspath("/workspace/isaaclab/assets/Factory_new/industreal_round_peg_16mm.usd")
print(f"Fixing peg USD file: {usd_path}")
...
peg_path = "/Root/industreal_round_peg_16mm"
peg_prim = stage.GetPrimAtPath(peg_path)
if not peg_prim or not peg_prim.IsValid():
    print(f"Error: {peg_path} prim not found")
    simulation_app.close()
    exit(1)

if not UsdPhysics.RigidBodyAPI.Get(stage, peg_path):
    UsdPhysics.RigidBodyAPI.Apply(peg_prim)
    print(f"Applied RigidBodyAPI to {peg_path}")
else:
    print(f"RigidBodyAPI already present on {peg_path}")

if not UsdPhysics.ArticulationRootAPI.Get(stage, peg_path):
    UsdPhysics.ArticulationRootAPI.Apply(peg_prim)
    print(f"Applied ArticulationRootAPI to {peg_path}")
else:
    print(f"ArticulationRootAPI already present on {peg_path}")
...
print("  - /Root/industreal_round_peg_16mm is RigidBody + ArtRoot")
